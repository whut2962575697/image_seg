...
from .modules.backbones.resnet_ibn import resnet34_ibn_a, resnet18_ibn_a, resnet34_ibn_b, resnet18_ibn_b, resnet101_ibn_a,resnet101_ibn_b
...
class BasicConv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, act_fn=nn.ReLU(inplace=True)):
...
class Res_Unet(nn.Module):
    def __init__(self, in_channels, n_classes, backbone, model_path, dropout_rate=0.0, scse=False, use_mish=False, db_block=False, hypercolumn=False, use_cc=False, deepsupversion=False, decode_dim=256):
        super().__init__()
        ...
        self.decode4 = Decoder(self.decode_dim, filters[2]+self.decode_dim, self.decode_dim, scse=scse, act_fn=self.act_fn)
        self.decode3 = Decoder(self.decode_dim, filters[1]+self.decode_dim, self.decode_dim, scse=scse, act_fn=self.act_fn)
        self.decode2 = Decoder(self.decode_dim, filters[0]+self.decode_dim, self.decode_dim, scse=scse, act_fn=self.act_fn)

        self.decode1 = Decoder(self.decode_dim, filter0+self.decode_dim, self.decode_dim, scse=scse, act_fn=self.act_fn)
        self.decode0 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(self.decode_dim, self.decode_dim // 2, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(self.decode_dim // 2),
            self.act_fn,
            nn.Conv2d(self.decode_dim // 2, self.decode_dim, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, input):
        ...
        eb2 = self.seb2([e2,F.upsample(f, scale_factor=4,  mode='bilinear', align_corners=False)], up_scale=False)
        d3 = self.decode3(d4, eb2) # 256,32,32

        eb1 = self.seb3([e1, F.upsample(f, scale_factor=8,  mode='bilinear', align_corners=False)], up_scale=False)
        d2 = self.decode2(d3, eb1) # 128,64,64

        eb0 = self.seb4([x, F.upsample(f, scale_factor=8,  mode='bilinear', align_corners=False)], up_scale=False)
        ...
        if self.deepsupversion:
            outs = [
                out,
                F.upsample(self.conv_last(d1), scale_factor=2, mode='bilinear', align_corners=False),
            ...
            outs = [
                ...
                F.upsample(self.conv_last(d3), scale_factor=4,  mode='bilinear', align_corners=False),
                F.upsample(self.conv_last(d4), scale_factor=8,  mode='bilinear', align_corners=False),
                # F.upsample(self.conv_last(f), scale_factor=16,  mode='bilinear', align_corners=False),
                # ppm_d3,
                # ppm_d4
